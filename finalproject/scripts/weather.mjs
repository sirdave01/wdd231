...
export async function initWeather() {
    ...
    try {
        ...
        currentEl.innerHTML = `
            <figure>
                <img src="https://openweathermap.org/img/wn/${cur.weather?.[0]?.icon}@2x.png" alt="${cur.weather?.[0]?.description || 'Weather icon'}" loading="lazy">
                <figcaption>${Math.round(cur.main.temp)}°C — ${cur.weather[0].description}</figcaption>
            </figure>`;

        // forecasting the next 3 days weather in Aba using its longitude and latitiude
        ...
        forecastEl.innerHTML = `<h3>3-Day Forecast</h3><div class="forecast-grid">
            ${daily.map(d => `
                <div class="day">
                    ...
                    <img src="https://openweathermap.org/img/wn/${d.weather?.[0]?.icon}.png" alt="${d.weather?.[0]?.description || 'Forecast icon'}" loading="lazy">
